...
func NewRouter(cfg config.Config, app *service.App) *Router {
	...
	api := r.Group("/api")
	{
		...
		authed.Use(middleware.Auth(app))
		{
			...
			authed.DELETE("/channels/:id", channelController.Delete)
			...
			// 通道账号
			authed.GET("/channel-accounts", channelAccountController.List)
			authed.POST("/channel-accounts", channelAccountController.Create)
