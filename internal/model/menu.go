...
// Menu 模型层：菜单与权限点，对应 menus 表。
// 结构字段用列存；前端展示扩展项放 Meta（JSON），改 menus.json 的 meta 即可透传。
type Menu struct {
	...
	Meta      JSONMap   `gorm:"column:meta;type:json" json:"meta"`                                         // Vben meta：hideInMenu / hideChildrenInMenu / affixTab 等，见 seed.go 注释
